...
public class Zoo {
    ...
    public void modifyMaintenance(){
        Mantenimiento maintenance = searchMaintenance(true);
        System.out.println("What would you like to modify?");
        System.out.println("1. Employee in charge");
        System.out.println("2. Treated animal");
        System.out.println("3. Process made");
        System.out.println("4. Process date");
        // Prints observations and then deletes according to id
        System.out.println("5. Add observation");
        System.out.println("6. Remove observation");
        System.out.print(">> ");
        int option = scanner.nextInt();
        scanner.nextLine();

        switch (option) {
            case 1: //Employee in charge
                Empleado employee;
                System.out.println("CURRENT EMPLOYEE: " + maintenance.getEmployeeNames());
                ...
                maintenanceList.add(maintenance);
                System.out.println("The employee in charce was succesfully changed!");
                returnToMainMenu();
                break;
        
            case 2: //Treated animal
                System.out.println("CURRENT ANIMAL: " + maintenance.getAnimalKind());
                Animal animal = searchAnimal(false);
                maintenance.setAnimal(animal);
                maintenanceList.add(maintenance);
                System.out.println("The treated animal was succesfully changed!");
                returnToMainMenu();
                break;
        
            case 3: //Process made
                System.out.println("CURRENT PROCESS: " + maintenance.getProcessMade());
                System.out.println("Please enter the new process:");
                System.out.print("Process:");
                String process = scanner.nextLine();
                maintenance.setProcess(process);
                maintenanceList.add(maintenance);
                System.out.println("The process made was succesfully changed!");
                returnToMainMenu();
                break;
        
            case 4: //Process date
                System.out.println("CURRENT PROCESS DATE: " + maintenance.getProcessDate().toString());
                LocalDate date = askForDate();
                maintenance.setDate(date);
                maintenanceList.add(maintenance);
                System.out.println("The process date was succesfully changed!");
                returnToMainMenu();
                break;
        
            case 5: //Add observation
                ArrayList<String> observations = new ArrayList<>();
                System.out.println("Please add the new observations to add:");
                System.out.println("Type 'exit' when finished");
                int obsNum = 0;
                while(true){
                    obsNum++;
                    System.out.println("New observation #" + obsNum);
                    String observation = scanner.nextLine();
                    if(observation.equalsIgnoreCase("exit")){
                        maintenance.addToObservations(observations);
                        maintenanceList.add(maintenance);
                        System.out.println("All the new observations were added to the observation list!");
                        returnToMainMenu();
                        break;
                    }else{
            ...
            case 6: //Remove observation
                maintenance.printObservationList();
                System.out.println("Please enter the ID of the observation to remove:");
                int id;
                while(true){
                    System.out.print("ID: ");
                    id = scanner.nextInt();
                    scanner.nextLine();
                    if(id>maintenance.observations.size() || id<0){
                        System.out.println("The entered ID is not valid.");
                        System.out.println("Please enter a valid ID between 0 and " + maintenance.observations.size());
                    }else{
                        break;
                    }
                }
                maintenance.removeFromObservationList(id);
                maintenanceList.add(maintenance);
                System.out.println("The selected observation was succesfully removed!");
                returnToMainMenu();
                break;
    ...
    public void deleteMaintenance(){
        ...
        System.out.println("Are you sure you want to remove this maintenance record? (y/n)");
        System.out.print(">> ");
        char option = scanner.nextLine().charAt(0);
        boolean answer = yesOrNo(option);
        if(answer){
            maintenanceList.remove(maintenance);
            System.out.println("The maintenance record was succesfully removed");
        }else{
            System.out.println("The maintenance record was not removed!");
        }
        returnToMainMenu();
    }
